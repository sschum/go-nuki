...
func (c ConfigSmartLockCommand) String() string {
	return fmt.Sprintf("Auto unlatch: %v; LED brightness: %d; Single lock: %v; HomeKit status: 0x%02x",
		c.AutoUnlatch(),
		c.LEDBrightness(),
		c.SingleLock(),
		c.HomeKitStatus(),
...
func (c ConfigOpenerCommand) String() string {
	return fmt.Sprintf("Capabilities: 0x%02x; Operation mode: 0x%02x",
		c.Capabilities(),
		c.OperationMode(),
	)
}
...
func (c ConfigCommand) String() string {
	...
	return fmt.Sprintf("Nuki-ID: %08x; Name: %s; Latitude: %f; Longitude: %f; "+
		"Pairing enabled: %v; Button enabled: %v; LED enabled: %v; Current Time (UTC): %s, TZ-Offset: %s; DST-Mode: 0x%02x; "+
		"Has Fob: %v; Fob Action#1: 0x%02x; Fob Action#2: 0x%02x; Fob Action#3: 0x%02x; "+
		"Advertising mode: 0x%02x; Has Keypad: %v; Firmware: %s; Hardware: %s; Timezone: %s; "+
		"%s",
		c.NukiId(),
		c.Name(),
		c.Latitude(),
